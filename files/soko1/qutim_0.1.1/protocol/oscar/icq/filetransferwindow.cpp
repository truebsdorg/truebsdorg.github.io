...
fileTransferWindow::fileTransferWindow(const QString &muin, const QStringList &list, const QString & uin,QByteArray & c,bool f, QWidget *parent)
    : QWidget(parent), sendingFile(f), cookie(c), contactUin(uin), fileList(list), mineUin(muin)
{
	ui.setupUi(this);
...
//					ui.currentFileLabel->setText(currentFile.fileName());
		...
		if ( tcpSocket->bytesAvailable()>=256 && waitingForGoodTransfer)
		{
			...
			if ( transferData.at(6) == 0x02 && transferData.at(7) == 0x04 )
			{
				...
				ui.cancelButton->setIcon(QIcon(":/icons/crystal_project/apply.png"));
...
//			tcpSocket->close();
			...
			tcpServer->listen(QHostAddress::Any, 5191);
			emit sendRedirectToMineServer(cookie, contactUin, 5191);
		}
	}
}
