...
//			return length;
		...
		} else if (msgType == 0x25)
		{
			...
			msg = "<qimage>" + extentionData.left(messageLength).toHex() + "</qimage>";
...
void icqMessage::sendImage(QTcpSocket *tcpSocket, const QString &uin, const QString &path, quint16 flapSeq,
			quint32 snacSeq)
{
	QByteArray packet;
	packet[0] = 0x2a;
	...
	* QTime::currentTime().second() * QTime::currentTime().msec();
	...
	{
		extentionData.append(convertLEToByteArray((quint16)(image.size())));
		extentionData.append(image.readAll());
		image.close();
	} else {
		extentionData.append(convertLEToByteArray((quint16)1));
		extentionData.append(convertToByteArray((quint8)0));
...
void icqMessage::sendXstatusReply(QTcpSocket *tcpSocket, const QString &uin,quint16 flapSeq, quint32 snacSeq)
{
...
void icqMessage::requestXStatus(QTcpSocket *tcpSocket, const QString &uin,quint16 flapSeq, quint32 snacSeq)
{
	QByteArray packet;
	packet[0] = 0x2a;
	packet[1] = 0x02;
