...
: QWidget(parent)
{
	...
	setWindowIcon(QIcon(":/icons/crystal_project/multiple.png"));
	move(desktopCenter());
...
void multipleSending::on_sendButton_clicked()
{
	...
	for( int i = 0; i < rootItem->childCount(); i++)
	{
		...
		{
			if ( !group->child(j)->toolTip(0).isEmpty() && group->child(j)->checkState(0))
						sendToList<<group->child(j)->toolTip(0);
		}
